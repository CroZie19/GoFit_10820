<template>
                    ...
                    <v-container>
                        <v-form ref="form">
                            <v-row>
                                ...
                                ></v-text-field>
                                <v-text-field
                                    v-model="form.tanggal_kelas_harian"
                                    :error-messages="errors.tanggal_kelas_harian"
                                    hidden
                                ></v-text-field>
                                <v-col cols="12" >
                                ...
                                <v-col cols="12" >
                                    <label
                                        ...
                                        >Pilih Instruktur Pengganti<span class="text-red-700">*</span></label
                                    >
                                    <select
                                        id="nama_pengganti"
                                        v-model="form.nama_pengganti"
                                        name="nama_pengganti"
                                        autocomplete="nama_pengganti"
                                        class="border border-dark w-100 custom-select custom-select-lg"
                                    >
                                        <option
                                            v-for="item in instrukturs"
                                            v-bind:key="item.id"
                                            :value="item.id"
                                        >
    ...
    <v-container>
        <v-data-table :headers="headers" :items="jadwalHarian" :search="search" item-value="name"
            class="elevation-1 rounded rounded-lg sha" @update:options="getJadwalHarianList"
            @update:instrukturs="getJadwalHarianList">
            <template v-slot:item="{ item }">
                <tr>
                    <td>{{ item.columns.nama_instruktur }}</td>
                    <td>{{ item.columns.jenis_kelas }}</td>
                    <td>
                        <span v-if="item.columns.status_kelas_harian == 'Kelas Bertugas'" class="text-success">{{ item.columns.status_kelas_harian }}</span>
...
export default {
    ...
    methods: {
        showDialog(type, id) {
            if (type == "Add") {
                this.dialogTitle = "Tambah Jadwal";
                this.form = {};
            } else if (type == "Edit") {
                this.dialogTitle = "Edit Jadwal";
                UserService.getJadwalHarian(id).then((response) => {
        ...
        submitForm() {
            ...
            if (this.dialog) {
                this.getJadwalHarianList();
            }
        },
        getJadwalHarianList() {
            UserService.getJadwalHarianList().then((response) => {
                if (response.data.success) {
                    this.jadwalHarian = response.data.data;
                }
            });
        },
        deleteInstruktur(id) {
